...
export const getGigTitles = query({
    args: {
        search: v.optional(v.string()), // Optional search parameter to filter titles
    },
    handler: async (ctx, args) => {
        const titleSearch = args.search as string;
        ...
        // Extract only the titles from the gigs
        const titles = gigs.map((gig) => gig.title);
